...
def cli():
    ...
    choice = questionary.autocomplete("Quelle commande voulez-vous exécuter ?", choices=commands_list).ask()
    while choice not in commands_list:
        print(Fore.YELLOW + "La commande saisie est incorrecte" + Fore.RESET)
        print("")
        choice = questionary.autocomplete("Quelle commande voulez-vous exécuter ?", choices=commands_list).ask()
