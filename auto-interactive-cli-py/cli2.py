...
def cli():
    ...
    for cmd in range(len(commands_list)):
        print(f"*  {commands_list[cmd]}")
    print("")

    choice = questionary.autocomplete("Which command do you want to run ?", choices=commands_list).ask()
    while choice not in commands_list:
        print(Fore.YELLOW + "The command entered is incorrect" + Fore.RESET)
        print("")
        choice = questionary.autocomplete("Which command do you want to run ?", choices=commands_list).ask()
...
if __name__ == "__main__":
    print(Fore.YELLOW + """--- START OF THE PROGRAM ---""" + Fore.RESET)
